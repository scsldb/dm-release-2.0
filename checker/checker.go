...
func (c *Checker) Init(ctx context.Context) (err error) {
	...
	for _, instance := range c.instances {
		...
		if checkSchema {
			c.checkList = append(c.checkList, check.NewTablesChecker(instance.sourceDB.DB, instance.sourceDBinfo, checkTables, false))
		}
	}
	...
	if checkingShard {
		for name, shardingSet := range sharding {
			...
			c.checkList = append(c.checkList, check.NewShardingTablesCheck(name, dbs, shardingSet, columnMapping, checkingShardID, false))
		}
	}
